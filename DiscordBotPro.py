...
class UpdaterClass():
    def __init__(self, currentversion):
        self.cversion = currentversion #Program version
        self.updatedurl = "https://www.dropbox.com/sh/7w3bs1y09qptam3/AAC6CyvmRS_FGxPWJfHaBPf5a?dl=1"
        self.lversionurl = "https://www.dropbox.com/s/0h1536z1w0skdqp/Version.txt?dl=1"
    
    def check_for_update(self): #Returns True if update is available
        try:
            import urllib.request
            self.content = urllib.request.urlopen(self.lversionurl)
            self.read_content = self.content.read()
            self.latestversion = str(self.read_content)[2:len(str(self.read_content))-1]
            if self.cversion < self.latestversion:
                return True
            else:
                return False
        except:
    ...
    def download_update(self): #Returns True if update was downloaded
        if self.check_for_update:
            try:
                import os
                os.system("start " + self.updatedurl)
                shutdown_program()
                return True
            except:
                return False

updatecheck = UpdaterClass(VERSION)
update_available = updatecheck.check_for_update()
...
def create_help_file():
    try:
        open("help.txt", "w").close()
    except:
        open("help.txt", "a").close()
    with open("help.txt", "w") as hfile:
        with open(resource_path(HELPFILE), "r") as thfile:
            hfile.write(thfile.read())

#GUI Setup

mainwindow = Tk()
mainwindow.title("Discord Bot")
...
if update_available:
    updatebtn = Button(mainwindow, text = "Update", bg = "#A2A2FF", activebackground = "#FFA2A2", command = updatecheck.download_update)
    updatebtn.place(x = 150, y = 5)
